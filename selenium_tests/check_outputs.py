...
def test_insert_NON_EXISTING_ESTATE_AND_MARKET_expected_csv_with_0_elements():
    ...
    try:
        ...
        pass
    bot_test.tester(estados=['AW'], mercados=['Agronegócio'], stacks=[], name=out_name)
    assert len(pd.read_csv(home + "/Downloads/" + out_name + ".csv").index) == 0
...
def test_insert_DF_AND_Agronegocio_AND_NON_EXISTING_STACK_expected_csv_with_0_elements():
    ...
    try:
        ...
        pass
    bot_test.tester(estados=['DF'], mercados=['Agronegócio'], stacks=['Luiz Felipe'], name=out_name)
    assert len(pd.read_csv(home + "/Downloads/" + out_name + ".csv").index) == 0
...
def test_insert_NOTHING_expected_no_csv():
    ...
    try:
        ...
        pass
    bot_test.tester(estados=[], mercados=[], stacks=[], name=out_name)
    assert exists(home + "/Downloads/" + out_name + ".csv") == False
